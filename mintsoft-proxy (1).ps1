...
Add-Type -AssemblyName System.Web  # needed for HttpUtility.ParseQueryString
...
while ($listener.IsListening) {
  ...
  # --- GitHub raw file download (for large files >1MB that Content API omits) ---
  if ($path.StartsWith("/raw-gh")) {
    $query  = [System.Web.HttpUtility]::ParseQueryString($req.Url.Query)
    $target = $query["url"]
    if (-not $target) { $res.StatusCode = 400; $res.Close(); continue }
    try {
      [System.Net.HttpWebRequest]$hw = [System.Net.WebRequest]::Create($target)
      $hw.Method    = "GET"
      $hw.UserAgent = "MintSoft-KPI-Proxy/1.0"
      $hw.Timeout   = 30000
      $hw.AutomaticDecompression = [System.Net.DecompressionMethods]::GZip -bor [System.Net.DecompressionMethods]::Deflate
      $auth = $req.Headers["Authorization"]
      if ($auth) { $hw.Headers["Authorization"] = $auth }
      $fwdRes = $null
      try   { $fwdRes = $hw.GetResponse() }
      catch [System.Net.WebException] { $fwdRes = $_.Exception.Response }
      Invoke-Upstream $res $fwdRes
    } catch {
      Write-Host "Raw-GH fatal: $_" -ForegroundColor Red
